...
public class ThumbnailManager extends ViewManager implements OnClickListener,
    ...
    private RotateImageView mDefaultThumbnail;
    ...
    private static final int UPDATE_THUMBNAIL_DELAY_INT = 1 * 1000;
    ...
    protected View getView() {
        ...
        mDefaultThumbnail = (RotateImageView) view.findViewById(R.id.thumbnail_default);
        mDefaultThumbnail.setOnClickListener(this);
        return view;
    }
        ...
        if (mThumbnailView != null) {
            if (isShowing()) {
                if (mThumbnail != null && mThumbnail.getBitmap() != null) {
                    ...
                    mDefaultThumbnail.setVisibility(View.GONE);
                } else {
                    //modify by ChMX
                	//mThumbnailView.setBitmap(BitmapFactory.decodeResource(mContext.getResources(), R.drawable.default_thumb));
                	//mThumbnailView.setVisibility(View.VISIBLE);
                	mDefaultThumbnail.setVisibility(View.VISIBLE);
...
//        updateThumbnailView();
    ...
    public void onFileSaved(SaveRequest request) {
        ...
        if (!request.isIgnoreThumbnail() && request.getUri() != null) {
            ...
            if (request.isContinuousRequest()) {
                mWorkerHandler.sendEmptyMessageDelayed(MSG_SAVE_THUMBNAIL, UPDATE_THUMBNAIL_DELAY_INT);
            } else {
    ...
    public void onClick(View v) {
        ...
        if (getContext().isFullScreen() && getContext().isCameraIdle()) {  //modify by ChMX
