...
import com.android.camera.mobile.view.MobileFrameLayout;
...
import com.ntian.adapter.NtianGridViewAdapter;
 ...
 * --------MavActor
 */
public class Camera extends ActivityBase implements
		PreviewFrameLayout.OnSizeChangedListener, LocationManager.Listener,
	...
	public static final int STATE_FOCUSING = 2; // Focus is in progress. The
												// exact focus state is in
												// Focus.java.
	public static final int STATE_SNAPSHOT_IN_PROGRESS = 3;
	public static final int STATE_RECORDING_IN_PROGRESS = STATE_SNAPSHOT_IN_PROGRESS; // share
																						// the
																						// same
																						// value
	public static final int STATE_SWITCHING_CAMERA = 4;// Switching between
														// cameras.
	public static final int STATE_PREVIEW_START_ING = 5;// add this state to
														// work wrong for
														// Gallery,when back key
														// press after onPause
	...
	private static final int MSG_SET_PREVIEW_MY_RATIO = 18;
	...
	private static final int SHOW_ONSCREEN_CS_SPPED_VIEW_POST_DELAY = 30;
	...
	AnimationDrawable gpsStart;

	// add by ChMX for mantis 712
	private static final int mDisableFlashBatteryLevel = 3;
	private int mRealLevel = -1;
	public static boolean mDisableFlash = false;
	private static final int MSG_DISABLE_FLASH = 20;
	AlertDialog mFlashDisableAD;
		...
		public void handleMessage(Message msg) {
			...
			case MSG_CHECK_DISPLAY_ROTATION:
				...
				if (SystemClock.uptimeMillis() - mOnResumeTime < 5000) {
					...
					MMProfileManager
							.triggersSendMessage(getMsgLabel(MSG_CHECK_DISPLAY_ROTATION)
									+ ", delayed 100");
				}
			...
			case MSG_UPDATE_SWITCH_ACTOR_STATE:
				...
				break;
			case MSG_SET_PREVIEW_MY_RATIO:
				setPreviewRatio();
				break;
			case MSG_DISABLE_FLASH: // add by ChMX for mantis 712
				disableFlash();
				break;
			default:
				break;
	...
	private class CameraStartUpThread extends Thread {
		...
		public void run() {
			...
			if (mCameraId != cameraId) {
				Log.w(TAG, "CameraStartUpThread.run() camera id preference="
						+ cameraId + ", memory=" + mCameraId);
				CameraSettings.writePreferredCameraId(mPreferences, mCameraId);
				// in order to do filterUnsuportedPreference in
			...
			try {
				...
				MMProfileManager
						.triggersSendMessage(getMsgLabel(MSG_OPEN_CAMERA_FAIL));
				...
				MMProfileManager
						.triggersSendMessage(getMsgLabel(MSG_OPEN_CAMERA_DISABLED));
				MMProfileManager.stopProfileCameraStartUp();
				return;
			}
			...
			MMProfileManager
					.triggersSendMessage(getMsgLabel(MSG_CAMERA_OPEN_DONE));
	...
	private void waitCameraStartUpThread(boolean cancel) {
		Log.i(TAG, "waitCameraStartUpThread(" + cancel
				+ ") begin mCameraStartUpThread=" + mCameraStartUpThread);
		try {
	...
	private void applyParameters(boolean force) {
		...
		lockRun(new Runnable() {
			@Override
			public void run() {
				// preview size changed, zsd changed, camera mode changed, open
				// camera.
				mSettingChecker.applyPreferenceToParameters();
				// Some preference are not set to native,but must do
				// applyPreferenceToParameters
		...
		// in full screen, in order to improve launch performance(reduce
		// RelativeFrame change animation)
		...
		final boolean changedPreviewSize = !curPreviewSize
				.equals(mLastPreviewSize);
		// camera screenail size will changed if preview ratio change
		boolean previewRatioChanged = false;
		if (mLastPreviewSize != null) {
			previewRatioChanged = !(((double) curPreviewSize.width / curPreviewSize.height) == ((double) mLastPreviewSize.width / mLastPreviewSize.height));
		}
		final boolean changedPictureSize = !curPictureSize
				.equals(mLastPictureSize);
		final boolean changedZsd = curZsd == null ? mLastZsdMode != null
				: !curZsd.equals(mLastZsdMode);
		final boolean needRestart = changedZsd || changedPreviewSize || force;
		boolean vBRateChanged = false;
		boolean aBRateChanged = false;
		if (mProfile != null) {
			vBRateChanged = (mLastVideoBitRate == UNKNOWN ? true
					: mLastVideoBitRate != mProfile.videoBitRate);
			aBRateChanged = (mLastAudioBitRate == UNKNOWN ? true
		...
		if (needRestart) {
			...
			// video wallpaper launch camera, the preview size is not changed,
			// not need copy texture.
			if (!isSwitchingCamera() && previewRatioChanged
					&& mSurfaceTexture != null && !isVideoWallPaperIntent()) {
		...
		lockRun(new Runnable() {
			@Override
			public void run() {
				setRotationToParameters(); // set rotation to parameters for
											// face detection.
				if (isSwitchingCamera() && (mZoomManager != null)) { // add by
																		// ChMX
																		// to
																		// reset
																		// zoom
				...
				setZoomParameter(); // maintain last ZoomValue to parameters
									// after resume
				applyParametersToServer();
				if (changedPreviewSize) {
					// For picture: Zoom related settings will be changed for
					// different preview
					// sizes, so set and read the parameters to get latest
					// values
		...
		MMProfileManager
				.triggersSendMessage(getMsgLabel(MSG_CAMERA_PARAMETERS_READY));
		mCameraActor.onCameraParameterReady(needRestart);
		if ((changedPictureSize || changedPreviewSize)
				|| (force || vBRateChanged || aBRateChanged)) {
		...
		Log.i(TAG,
				"applyParameters(" + force + ") picturesize="
						+ SettingUtils.buildSize(curPictureSize)
						+ " previewsize="
						+ SettingUtils.buildSize(curPreviewSize)
						+ " oldPictureSize="
						+ SettingUtils.buildSize(mLastPictureSize)
						+ " oldPreviewSize="
						+ SettingUtils.buildSize(mLastPreviewSize)
						+ " changedPreviewSize=" + changedPreviewSize
						+ ", changedPictureSize=" + changedPictureSize
						+ " oldZsd=" + mLastZsdMode + ", curZsd=" + curZsd
						+ ", changedZsd=" + changedZsd + " vBRateChanged="
						+ vBRateChanged + ", aBRateChanged=" + aBRateChanged);
	...
	private boolean cancelApplyParameters() {
		boolean cancel = (mCameraDevice == null || mParameters == null
				|| mPaused || (mCameraStartUpThread != null && mCameraStartUpThread
				.isCanceled()));
		if (cancel) {
			Log.i(TAG, "cancelApplyParameters() mCameraDevice=" + mCameraDevice
					+ ", " + "mParameters=" + mParameters + ", mPaused="
					+ mPaused + " return " + cancel);
		}
		return cancel;
	}
	...
	public boolean isCameraIdle() {
		boolean idle = (mCameraState == STATE_IDLE)
				|| ((mFocusManager != null) && mFocusManager.isFocusCompleted() && (mCameraState != STATE_SWITCHING_CAMERA));
		Log.d(TAG, "isCameraIdle() mCameraState=" + mCameraState + ", return "
				+ idle);
		return idle;
	}
	...
	private boolean isNeedToSetLandScape() {
		if (FeatureSwitcher.isSmartBookEnabled()) {
			return getCurrentMode() == ModePicker.MODE_MOTION_TRACK
					|| getCurrentMode() == ModePicker.MODE_MAV;
		} else {
			return false;
		}
	}
	...
	public void onCreate(Bundle icicle) {
		...
		if (isNonePickIntent()) {
			CameraSettings.updateSettingCaptureModePreferences(mPreferences
					.getLocal());
		}
		// @}
		...
		} else if (isStereo3DImageCaptureIntent()
				&& FeatureSwitcher.isStereoSingle3d()) {
		...
		MobileFrameLayout layout = (MobileFrameLayout) findViewById(R.id.root);
	...
	private void initializeForOpeningProcess() {
		...
		if (FrameworksClassFactory.isMockCamera()) {
			mPreviewFrameLayout
					.setBackgroundResource(R.drawable.mock_preview_rainbow);
		}
	...
	// add by ChMX to set preview layout ratio
	private void setPreviewRatio() {
		if ((mSettingChecker != null) && (mPreviewFrameLayout != null)) {
			String CurrentValue = mSettingChecker
					.getSettingCurrentValue(SettingChecker.ROW_SETTING_PICTURE_RATIO);
			if (CurrentValue != null) {
				double ratio = Double.parseDouble(CurrentValue);
				if (ratio > 0) {
					mPreviewFrameLayout.setAspectRatio(ratio);
				} else {
					mPreviewFrameLayout.setAspectRatio(4.0 / 3.0);
				}
			} else {
				mPreviewFrameLayout.setAspectRatio(4.0 / 3.0);
			}
	...
	private void screenOnSmartBook() {
		Log.d(TAG,
				"screenOnSmartBook,FO is :"
						+ FeatureSwitcher.isSmartBookEnabled());
		// add for SMARTBOOK
		if (FeatureSwitcher.isSmartBookEnabled()) {
	...
	protected void onStart() {
		...
		filter.addAction("com.android.systemui.PANEL_VISIBLE");
		filter.addAction("com.android.systemui.PANEL_INVISIBLE");
		registerReceiver(mStatusBarReceiver, filter);
	}
	...
	protected void onStop() {
		...
		if ((mShutterManager != null) && setOff) {
			mShutterManager.setBackKeyBrightness(true);
		}
	}
	...
	BroadcastReceiver mStatusBarReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals("com.android.systemui.PANEL_VISIBLE")) {
				if ((mShutterManager != null)
						&& ((mShutterManager.getShutterType() == ShutterManager.SHUTTER_TYPE_OK_CANCEL) || (mShutterManager
								.getShutterType() == ShutterManager.SHUTTER_TYPE_CANCEL))) {
					mShutterManager.setBackKeyBrightness(true);
					setOff = false;
				}
			} else if (action.equals("com.android.systemui.PANEL_INVISIBLE")) {
				if ((mShutterManager != null)
						&& ((mShutterManager.getShutterType() == ShutterManager.SHUTTER_TYPE_OK_CANCEL) || (mShutterManager
								.getShutterType() == ShutterManager.SHUTTER_TYPE_CANCEL))) {
					mShutterManager.setBackKeyBrightness(false);
					setOff = true;
				}
				if (!isLocationEnabled()) {
					if (mPreferences != null) {
						SharedPreferences.Editor editor = mPreferences.edit();
						editor.putString(CameraSettings.KEY_RECORD_LOCATION,
								"off");
						editor.commit();
					}
					if (mLocationManager != null) {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				if (scale == 100) {
					...
					mRealLevel = (int) (level * 100 / scale);
				}
				updateFlashState();
			}
		}
	...
	// add by ChMX for mantis 712
	private void registermBatteryChanged() {
		IntentFilter filter = new IntentFilter();
		filter.addAction(Intent.ACTION_BATTERY_CHANGED);
		registerReceiver(mBatteryChanged, filter);
	}

	// add by ChMX for mantis 712
	private void unregistermBatteryChanged() {
		unregisterReceiver(mBatteryChanged);
	}

	// add by ChMX for mantis 712
	private void disableFlash() {
		if (mPickerManager != null && isCameraViewVisible()) {
			mDisableFlash = true;
			mPickerManager.disableFlashState();
		} else {
			mDisableFlash = false;
		}
	}
	...
	// add by ChMX for mantis 712
	@Override
	public void cameraAppViewUpdate() {
		updateFlashState();
	}

	// add by ChMX for mantis 712
	private void updateFlashState() {
		if (!mDisableFlash && mRealLevel >= 0
				&& mRealLevel < mDisableFlashBatteryLevel) {
			mDisableFlash = true;
			mMainHandler.removeMessages(MSG_DISABLE_FLASH);
			mMainHandler.sendEmptyMessageDelayed(MSG_DISABLE_FLASH, 600);
		}
		if (mDisableFlash && mRealLevel > mDisableFlashBatteryLevel) {
			mDisableFlash = false;
			mMainHandler.removeMessages(MSG_DISABLE_FLASH);
			if (mFlashDisableAD == null && mFlashDisableAD.isShowing()) {
				mFlashDisableAD.dismiss();
			}
		}
	}

	// add by ChMX for mantis 712
	private boolean isCameraViewVisible() {
		if (mCameraAppView != null) {
			return mCameraAppView.getVisibility() == View.VISIBLE;
		}
		return false;
	}

	// add by ChMX for mantis 712
	private void showFlashDisableAD() {
		if (mFlashDisableAD == null) {
			mFlashDisableAD = new AlertDialog.Builder(this)
					.setTitle(getResources().getString(R.string.alert_warning))
					.setIconAttribute(android.R.attr.alertDialogIcon)
					.setMessage(
							getResources().getString(
									R.string.falsh_disable_message))
					.setPositiveButton(
							getResources().getString(
			...
			mFlashDisableAD.show();
		} else if (!mFlashDisableAD.isShowing()) {
			mFlashDisableAD.show();
		}
	}
	...
	protected void onResume() {
		Log.i(TAG, "onResume() mForceFinishing=" + mForceFinishing
				+ ", mOpenCameraFail=" + mOpenCameraFail + ", mCameraDisabled="
				+ mCameraDisabled + ", mCameraState=" + mCameraState
	...
	public void screenOffForSmartBook() {
		Log.d(TAG, "screenOffForSmartBook, mIsSmartBookPlugged = "
				+ getSmartBookPluggedState() + ",mPowerManager = "
				+ mPowerManager);
	...
	private class SmartBookBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_SMARTBOOK_PLUG.equals(action)) {
				mIsSmartBookPlugged = intent.getBooleanExtra(
	...
	public void onBackPressed() {
		...
		/*
		 * we should first check RotateDialog and SettingManager's Statusback
		 * key should not do in CameraActor when RotateDialog and SettingManager
		 * is alive
		 */
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		Log.d(TAG, "mCameraState = " + getViewState() + ",isSettingsView = "
				+ isSettingsViewState);
		clearFocusAndFace();
		mRotateToast = null;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		// add by ChMX for GPS after set location in Settings
		if (requestCode == 200) {
			SharedPreferences.Editor editor = mPreferences.edit();
			if (isLocationEnabled()) {
				editor.putString(CameraSettings.KEY_RECORD_LOCATION, "on");
			} else {
				editor.putString(CameraSettings.KEY_RECORD_LOCATION, "off");
			}
			editor.commit();
			notifyPreferenceChanged(null);
			mLocationManager.recordLocation(RecordLocationPreference.get(
	...
	private void setPreviewFrameLayoutAspectRatio() {
		...
		if (mPreviewFrameLayout != null && mParameters != null) {
			...
			if (isVideoMode() && mProfile != null) {
				...
				// so preview layout ratio should equals full screen preview
				// ratio
			...
			Log.i(TAG, "setPreviewFrameLayoutAspectRatio() width=" + width
					+ ", height=" + height);
		}
	}
	...
	public void onFirstFrameArrived() {
		...
		// //Mark for some case onFristFrameArrived called from none-main
		// thread.
	...
	private void setLoadingAnimationVisible(boolean visible) {
		...
		/*
		 * View view = findViewById(R.id.camera_app_loading_animation); if (view
		 * != null) { if (visible) { view.setVisibility(View.VISIBLE); } else {
		 * Util.fadeOut(view); view.setVisibility(View.GONE); } }
		 */
	}
	...
	protected void onSingleTapUp(View view, int x, int y) {
		...
		if (!mRotateDialog.isShowing() && !mRotateProgress.isShowing()) { // we
																			// do
																			// nothing
																			// for
																			// dialog
			...
			if (!mSettingManager.collapse(true)) {
				if (mCameraActor.getonSingleTapUpListener() != null) {
					mCameraActor.getonSingleTapUpListener().onSingleTapUp(view,
							x, y);
					CameraInfo info = CameraHolder.instance().getCameraInfo()[mCameraId];
					if (mCameraActor.getSelfTimerManager() != null
							&& mCameraActor.getSelfTimerManager()
									.isSelfTimerCounting()) {
						return;
					}
		...
		if (!mRotateDialog.isShowing() && !mRotateProgress.isShowing()) { // we
			...
			if (!mSettingManager.collapse(true)) {
				if (mCameraActor.getonLongPressListener() != null) {
					mCameraActor.getonLongPressListener().onLongPress(view, x,
							y);
				}
			}
		}
	...
	public void setSurfaceTextureReady(boolean ready) {
		Log.i(TAG, "setSurfaceTextureReady(" + ready
	...
	public void setPreviewTextureAsync() {
		Log.i(TAG, "setPreviewTextureAsync() mSurfaceTextureReady="
				+ mSurfaceTextureReady + ", mSurfaceTexture=" + mSurfaceTexture);
	...
	private void updateCameraScreenNailSize(int oldWidth, int oldHeight,
			Size size) {
	...
	private void setDisplayOrientation() {
		...
		Log.i(TAG, "setDisplayOrientation() mDisplayRotation="
				+ mDisplayRotation + ", mCameraDisplayOrientation="
	...
	public int getPreviewFrameWidth() {
		Log.d(TAG, "getPreviewFrameWidth() return " + mPreviewFrameHeight
				+ ", real=" + mPreviewFrameLayout.getWidth());
		return mPreviewFrameWidth;
	}

	public int getPreviewFrameHeight() {
		Log.d(TAG, "getPreviewFrameHeight() return " + ", real="
	...
	public View inflate(int layoutId, int layer) {
		...
		return getLayoutInflater()
				.inflate(layoutId, getViewLayer(layer), false);
	}
	...
	public boolean removeOnFullScreenChangedListener(
			OnFullScreenChangedListener l) {
	...
	private void notifyParametersReady() {
		if ((isNonePickIntent()
				...
				&& getViewState() != VIEW_STATE_SETTING && getViewState() != VIEW_STATE_SUB_SETTING)
		...
		if (!isSecureCamera()) {
			...
			if (isLocationEnabled()) {
				...
				if (RecordLocationPreference.get(mPreferences,
						getContentResolver())) {
					SharedPreferences.Editor editor = mPreferences.edit();
					editor.putString(CameraSettings.KEY_RECORD_LOCATION, "off");
					editor.commit();
	...
	private void hideActorViews() {
		...
		Log.d(TAG, "hideActorViews() size=" + size + ", mCommonManagerCount="
				+ mCommonManagerCount);
	}

	private void restoreActorViews() {
		if (mLastManagers == null) {
			return;
		}
	...
	public boolean collapseViewManagerWithoutSettingManager(boolean force) {
		boolean handle = false;
		// hide dialog if it's showing.
		if (mRotateDialog.isShowing() && !force) {
			mRotateDialog.hide();
			...
			for (ViewManager manager : mViewManagers) {
				if (manager instanceof SettingManager) {
					continue; // ignore it
				}
				handle = manager.collapse(force) || handle;
				if (!force && handle) {
	...
	private ModePicker.OnModeChangedListener mModeChangedListener = new ModePicker.OnModeChangedListener() {
		...
		public void onModeChanged(int newMode) {
			...
			if (mCameraActor.getMode() != newMode) {
				...
				if (mPaused || mCameraState == STATE_SWITCHING_CAMERA) { // startup
																			// thread
																			// will
																			// apply
																			// these
				...
				int newCameraMode = SettingChecker.getCameraMode(mCameraActor
						.getMode());
				// switch live photo to video,need do restart preview
				boolean needRestart = ((oldCameraMode != newCameraMode) || SettingChecker
						.isVideoCameraMode(newCameraMode));
	...
	private CameraScreenNail.SwitchActorStateListener mStateListener = new CameraScreenNail.SwitchActorStateListener() {
		...
		public void onStateChanged(int state) {
			// TODO Auto-generated method stub
			// if MSG_UPDATE_SWITCH_ACTOR_STATE message is has been done in
			// during changing mode,
	...
	private void releaseCameraActor() {
		...
		collapseViewManagerWithoutSettingManager(true);
	...
	private void applayViewCallbacks() {
		mShutterManager.setShutterListener(mPhotoShutterListener,
				mVideoShutterListener, mCameraActor.getOkListener(),
	...
	private void applyDeviceCallbacks() { // should be checked
		if (mCameraDevice != null) {
			...
			mCameraDevice.setCShotDoneCallback(mCameraActor
					.getContinuousShotDone());
			mCameraDevice.setErrorCallback(mCameraActor.getErrorCallback());
			mCameraDevice.setFaceDetectionListener(mCameraActor
					.getFaceDetectionListener());
			mCameraDevice.setObjectTrackingListener(mCameraActor
					.getObjectTrackingListener());
	...
	private OnShutterButtonListener mVideoShutterListener = new OnShutterButtonListener() {
		...
		public void onShutterButtonLongPressed(ShutterButton button) {
			...
			OnShutterButtonListener listener = mCameraActor
		...
		public void onShutterButtonClick(final ShutterButton button) {
			Log.i(TAG, "Video.onShutterButtonClick(" + button
					+ ") isFullScreen()=" + isFullScreen()
					+ ",mCameraOpened = " + mCameraOpened
					+ ",mCameraStartUpThread =" + mCameraStartUpThread);
			...
			// do not call after onpause or before onresume's
			// mCameraStartUpThread start completely.
			if (isFullScreen() && mCameraOpened
					&& (mCameraStartUpThread == null)) {
				OnShutterButtonListener listener = mCameraActor
						.getVideoShutterButtonListener();
				if (listener != null) {
					...
					if (Storage.getLeftSpace() > 0) {
						mModePicker.setCurrentMode(ModePicker.MODE_VIDEO);// new
																			// video
																			// mode
	...
	public void backToLastMode() {
		...
		boolean visible = mLastMode == UNKNOWN ? false : ModeChecker
				.getModePickerVisible(this, getCameraId(), mLastMode);
		if (!visible) { // if current camera not support current mode, change it
						// to default.
	...
	private void initializeFocusManager() {
		...
		String[] defaultFocusModes = SettingChecker.getModeDefaultFocusModes(
				this, mCameraActor.getMode());
		mFocusManager = new FocusManager(
				...
				mFocusAreaIndicator,
				mInitialParams,
				mCameraActor.getFocusManagerListener(),
				mirror,
				getMainLooper(),
				SettingChecker.getModeContinousFocusMode(mCameraActor.getMode()));
		mFocusManager.setPreviewSize(getPreviewFrameWidth(),
	...
	private void initializeCameraPreferences() {
		Log.i(TAG, "initializeCameraPreferences() mPreferenceGroup="
				+ mPreferenceGroup);
		MMProfileManager.startProfileInitPref();
		if (mPreferenceGroup == null) { // should be rechecked for switching
										// camera case
			mCameraSettings = new CameraSettings(this, mInitialParams,
					mCameraId, CameraHolder.instance().getCameraInfo());
			mPreferenceGroup = SettingChecker.filterUnsuportedPreference(
					mCameraSettings, mCameraSettings
							.getPreferenceGroup(R.xml.camera_preferences),
					mCameraId);
			limitSettingsByIntent();
			mMainHandler.sendEmptyMessage(MSG_SET_PREVIEW_MY_RATIO);
		}
		mMainHandler.sendEmptyMessage(MSG_CAMERA_PREFERENCE_READY);
		MMProfileManager
				.triggersSendMessage(getMsgLabel(MSG_CAMERA_PREFERENCE_READY));
	...
	// / M: setting related @{
	/*
	 * setRotation//for taken Utils.setGpsParameters//for taken
	...
	private void applyParameterForCapture(final SaveRequest request) {
		lockRun(new Runnable() {
			...
			public void run() {
				...
				int jpegRotation = Util
						.getJpegRotation(mCameraId, mOrientation);
	...
	public void applyContinousCallback() {
		...
		if (mCameraDevice != null) {
			if (mFocusManager.getContinousFocusSupported()) {
				mCameraDevice.setAutoFocusMoveCallback(mCameraActor
						.getAutoFocusMoveCallback());
			} else {
				mCameraDevice.setAutoFocusMoveCallback(null);
			}
		}
	...
	public SaveRequest preparePhotoRequest() {
		return preparePhotoRequest(Storage.FILE_TYPE_PHOTO,
	...
	// resolution
	public SaveRequest prepareVideoRequest(int fileType, int outputFileFormat,
			String resolution) {
		SaveRequest request = mFileSaver.prepareVideoRequest(fileType,
				outputFileFormat, resolution);
		return request;
	}

	// Fetch parameters from server for something changed by server.
	// Note: mCameraDevice.getParameters() consumes long time, please don't use
	// it unnecessary.
	public Parameters fetchParametersFromServer() {
		Log.i(TAG, "fetchParameterFromServer() mParameters=" + mParameters
	...
	private SettingManager.SettingListener mSettingListener = new SettingManager.SettingListener() {
		...
		public void onSharedPreferenceChanged(ListPreference preference) {
			Log.i(TAG, "onSharedPreferenceChanged ListPreference = "
					+ preference);
			// when choose smile_shot, hdr, asd setting, capture mode will be
			// changed.
			if (preference != null
					&& (preference.getKey().equals(
							CameraSettings.KEY_SMILE_SHOT)
							|| preference.getKey().equals(
									CameraSettings.KEY_HDR) || preference
							.getKey().equals(CameraSettings.KEY_ASD))) {
		...
		public void onRestorePreferencesClicked() {
			...
			Runnable runnable = new Runnable() {
				...
				public void run() {
					...
					collapseViewManager(true);
					CameraSettings.restorePreferences(Camera.this,
							mPreferences, mParameters, isNonePickIntent());
					mZoomManager.resetZoom();
					// we should apply parameters if mode is default too.
					if (ModePicker.MODE_PHOTO == mCameraActor.getMode()
							|| !isNonePickIntent()
							|| ModePicker.MODE_PHOTO_SGINLE_3D == mCameraActor
									.getMode()
							|| ModePicker.MODE_PHOTO_3D == mCameraActor
									.getMode()) {
						applyParameters(false);
					} else {
			...
			showAlertDialog(null, getString(R.string.confirm_restore_message),
	...
	public String getSelfTimer() {
		String seflTimer = mSettingChecker
				.getSettingCurrentValue(SettingChecker.ROW_SETTING_SELF_TIMER);
	...
	public CamcorderProfile fetchProfile(int quality, int timelapseMs) {
		Log.i(TAG, "fetchProfile(" + quality + ", " + timelapseMs
				+ " mCameraId = " + mCameraId + ")");
		...
		mProfile = FrameworksClassFactory.getMtkCamcorderProfile(mCameraId,
				quality);
		// revise video profile for operator.
		ExtensionHelper.getFeatureExtension().checkMMSVideoCodec(quality,
				mProfile);
		Log.i(TAG, "fetchProfile() mProfile.videoFrameRate="
				+ mProfile.videoFrameRate + ", mProfile.videoFrameWidth="
				+ mProfile.videoFrameWidth + ", mProfile.videoFrameHeight="
				+ mProfile.videoFrameHeight + ", mProfile.audioBitRate="
				+ mProfile.audioBitRate + ", mProfile.videoBitRate="
				+ mProfile.videoBitRate + ", mProfile.quality="
	...
	public boolean getMicrophone() {
		return CameraSettings.VIDEO_MICRIPHONE_ON.equals(mSettingChecker
				.getSettingCurrentValue(SettingChecker.ROW_SETTING_MICROPHONE));
	}

	public String getAudioMode() {
		return mSettingChecker
				.getSettingCurrentValue(SettingChecker.ROW_SETTING_AUDIO_MODE);
	}
	...
	public void setCameraRequestOrientaion(int orientationRequest) {
		Log.d(TAG, "lastTimeOrientation = " + lastTimeOrientation
				+ ",orientationRequest = " + orientationRequest + ",TID:"
				+ Thread.currentThread().getId());
		...
		if (!isAppGuideFinished()) {
			Log.d(TAG,
					"AppGuide is not Finished,don't set the requestorientation");
			return;
		}
	...
	private class MyOrientationEventListener extends OrientationEventListener {
		...
		public void onOrientationChanged(int orientation) {
			...
			if (mLock && !getSmartBookPluggedState()) {
				// Use case: MAV mode
				// We need unlock orientation when orientation is 90 or 270 and
				// than lock
			...
			int orientationCompensation = (mOrientation + Util
					.getDisplayRotation(Camera.this)) % 360;
			// add the isFullScreen() state is used when go to gallery rotate
			// the device
			// ,the activity will not be restarted[because we not set the
			// requesteorientation]
			if (getSmartBookPluggedState()
					&& FeatureSwitcher.isSmartBookEnabled() && isFullScreen()) {

				// follow is used for SMB connected and when check to the
				// mode:mav & motion track
				// when is this mode we will set the requestOrientation to 270
				// (landscape)
			...
			if (mOrientationCompensation != orientationCompensation) {
				...
				notifyOrientationChanged();
				if (isVideoWallPaperIntent() && !mReviewManager.isShowing()
						&& mCameraState == STATE_IDLE) {
					applyParameters(false); // / M: apply new preview size for
											// orientation.
				}
				lockRun(new Runnable() {
					@Override
	...
	public void setRotationToParameters() {
		...
		Log.i(TAG, "setRotationToParameters() mCameraId=" + mCameraId
				+ ", mOrientation=" + mOrientation + ", jpegRotation="
				+ jpegRotation);
	}
	...
	private void switchCamera() {
		Log.i(TAG, "switchCamera() begin id=" + mPendingSwitchCameraId
				+ ", mPaused=" + mPaused);
		if (mPaused || mOpenCameraFail || mCameraDisabled
				|| mPendingSwitchCameraId == UNKNOWN) {
			return;
		}
		...
		String value = mPreferences.getString(CameraSettings.KEY_GESTURE_SHOT,
				"off");
		// Restart the camera and initialize the UI. From onCreate.
		mPreferences.setLocalId(Camera.this, mCameraId);
		CameraSettings.upgradeLocalPreferences(mPreferences.getLocal());

		// capture mode should be reset when switch camara while
		// hdr/smileshot/asd is open on back camera.
		if (mModePicker != null) {
		...
		MMProfileManager
				.triggersSendMessage(getMsgLabel(MSG_SWITCH_CAMERA_START_ANIMATION));
		// end switching camera
		mPendingSwitchCameraId = UNKNOWN;
		refreshModeRelated();
		restoreViewState();

		// add by ChMX for mantis 803
		if (mDisableFlash && mCameraId == 0) {
	...
	public void animateCapture() {
		...
		boolean rotationLocked = (1 != Settings.System
		...
		Log.d(TAG, "animateCapture() rotation=" + rotation
	...
	protected void onPreviewTextureCopied() {
		...
		MMProfileManager.triggersSendMessage(getMsgLabel(MSG_SWITCH_CAMERA));
	}
	...
	private PickerManager.PickerListener mPickerListener = new PickerManager.PickerListener() {
		...
		public boolean onCameraPicked(int cameraId) {
			...
			if (mPaused
					|| mPendingSwitchCameraId != UNKNOWN
					|| !ModeChecker.getModePickerVisible(Camera.this, cameraId,
							getCurrentMode())) {
				return false;
			}
		...
		public boolean onFlashPicked(String flashMode) {
			...
			mMainHandler.sendEmptyMessage(MSG_APPLY_PARAMETERS_WHEN_IDEL);
			MMProfileManager
					.triggersSendMessage(getMsgLabel(MSG_APPLY_PARAMETERS_WHEN_IDEL));
			return true;
		}
	...
	private void parseIntent() {
		...
		if (MediaStore.ACTION_IMAGE_CAPTURE.equals(action)
			...
			mWallpaperAspectio = intent.getFloatExtra(
					EXTRA_VIDEO_WALLPAPER_RATION, WALLPAPER_DEFAULT_ASPECTIO);
		...
		if (mPickType != PICK_TYPE_NORMAL) {
			...
			mLimitedDuration = intent.getIntExtra(
	...
	private void limitPreviewByIntent(CamcorderProfile profile) {
		Log.d(TAG, "limitPreviewByIntent() videoFrameWidth="
				+ profile.videoFrameWidth + ", videoFrameHeight="
				+ profile.videoFrameHeight + ", mDisplayRotation="
				+ mDisplayRotation + ", mOrientationCompensation="
				+ mOrientationCompensation);
		if (isVideoWallPaperIntent() && mParameters != null) {
			final List<Size> previewSizes = new ArrayList<Size>(
					mParameters.getSupportedPreviewSizes());
			final int orientation = mOrientationCompensation;
			Size minSize = previewSizes.get(previewSizes.size() - 1); // last
																		// one
																		// is
																		// biggest
																		// one.
			...
			for (Iterator<Size> iter = previewSizes.iterator(); iter.hasNext();) {
				Size size = iter.next();
				if (size.width < WALLPAPER_MIN_WIDTH
						|| size.height < WALLPAPER_MIN_WIDTH) {
					continue;
				}
				float aspectRatio = (float) size.width / size.height;
				if (Math.abs(aspectRatio - mWallpaperAspectio) <= Math
						.abs(minAspectio - mWallpaperAspectio)) {
			...
			if (mDisplayRotation == 0) { // always width > height
				if (orientation % 180 == 0) {
					// profile.videoFrameWidth = minSize.height * minSize.height
					// / minSize.width;
			...
			Log.d(TAG,
					"limitPreviewByIntent() findSize="
							+ SettingUtils.buildSize(minSize));
		}
		Log.i(TAG, "limitPreviewByIntent() videoFrameWidth="
				+ profile.videoFrameWidth + " videoFrameHeight="
				+ profile.videoFrameHeight + ", mDisplayRotation="
	...
	private void limitSettingsByIntent() {
		...
		if (isImageCaptureIntent()) {
			mCameraSettings.removePreferenceFromScreen(mPreferenceGroup,
	...
	public void setViewState(int state) {
		Log.i(TAG, "setViewState(" + state + ") mCurrentViewState="
				+ mCurrentViewState + ", mPendingSwitchCameraId="
		...
		} else if (FeatureSwitcher.isTablet()
	...
	private String mCSInfoString = "";
	private Runnable mShowCSInfoRunnable = new Runnable() {
		@Override
		public void run() {
			mCsSpeedManager.showText(mCSInfoString);
			showIndicator(DELAY_MSG_SHOW_ONSCREEN_CS_SPEED_VIEW);
		}
	};

	private void doShowCSInfo(final String text, final int showMs) {
		mCSInfoString = text;
		mUiHandler.removeCallbacks(mShowCSInfoRunnable);
		mUiHandler.postDelayed(mShowCSInfoRunnable,
				SHOW_ONSCREEN_CS_SPPED_VIEW_POST_DELAY);
	}
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (mRemainingManager.isShowing()) {
					...
					mMainHandler
							.removeMessages(MSG_DELAY_SHOW_ONSCREEN_INDICATOR);
					mMainHandler.sendEmptyMessageDelayed(
							MSG_DELAY_SHOW_ONSCREEN_INDICATOR,
							DELAY_MSG_SHOW_ONSCREEN_TIME);
					mDelayShowInfo = text;
	...
	private void showIndicator(int delayMs) {
		...
		mMainHandler.removeMessages(MSG_SHOW_ONSCREEN_INDICATOR);
		if (delayMs > 0) {
			mMainHandler.sendEmptyMessageDelayed(MSG_SHOW_ONSCREEN_INDICATOR,
					delayMs);
		} else {
	...
	public void showAlertDialog(String title, String msg, String button1Text,
		...
		mRotateDialog.showAlertDialog(title, msg, button1Text, r1, button2Text,
				r2);
	}
	...
	// View state may be changed for many reasons, here we use special flag to
	// keep floating info.
	...
	public boolean isAcceptFloatingInfo() {
		...
		Log.d(TAG, "isAcceptFloatingInfo() isFullScreen()=" + isFullScreen()
				+ ", mAcceptFloatingInfo=" + mAcceptFloatingInfo + " return "
				+ accepted);
		return accepted;
	}
	...
	// add by ChMX start for GPS
	@Override
	public void startGps() {
		RotateImageView mGpsIndicator = mIndicatorManager.mGpsIndicator1;
		if (mGpsIndicator == null) {
			return;
		}
		mGpsIndicator.setBackgroundResource(R.anim.gpsanimation);
		gpsStart = (AnimationDrawable) mGpsIndicator.getBackground();
		gpsStart.start();
		mHasSignal = false;
	...
	public void showGpsOnScreenIndicator(boolean hasSignal) {
		// TODO Auto-generated method stub
		RotateImageView mGpsIndicator = mIndicatorManager.mGpsIndicator1;
		if (mGpsIndicator == null || mHasSignal == hasSignal) {
			return;
		}
		...
		if (mHasSignal) {
			mGpsIndicator
					.setBackgroundResource(R.drawable.camera_indicator_gps_connecting03);
		} else {
			mGpsIndicator
					.setBackgroundResource(R.drawable.camera_indicator_gps_no_signal);
		}
		mGpsIndicator.setVisibility(View.VISIBLE);
	}
	...
	private void initView() {
		mOptionGridView = (GridView) findViewById(R.id.option_grid);
		getOptionItemInfos();
		NtianGridViewAdapter mNtianGridViewAdapter = new NtianGridViewAdapter(
				Camera.this, optionButtonList);
		mOptionGridView.setAdapter(mNtianGridViewAdapter);

	}

	List<OptionButton> optionButtonList = new ArrayList<OptionButton>();
	// String[] optionNames = new
	// String[]{"全景","美颜","声控相机","HDR","连拍","夜景防抖","手动"};
	int[] optionNames = new int[] { R.string.all_view, R.string.beauty,
			R.string.voice_contral, R.string.HDR, R.string.cont_photo,
			R.string.night, R.string.hander };
	int[] optionIcons = new int[] { R.drawable.ntian_all_view_selector,
			R.drawable.ntian_pretty_selector, R.drawable.ntian_voice_selector,
			R.drawable.ntian_hdr_selector,
			R.drawable.ntian_count_photo_selector,
			R.drawable.ntian_night_selector, R.drawable.ntian_handler_selector };

	/**
	 * 获取option中的item信息
	 */
	private void getOptionItemInfos() {
		optionButtonList.clear();
		for (int i = 0; i < optionNames.length; i++) {
			OptionButton option = new OptionButton();
			option.setOptionName(getString(optionNames[i]));
			option.setDrawableId(optionIcons[i]);
